(() => {
  ...
  let enterNewScene = false; //새로운 scene이 시작된 순간 true로 변경하기위한 변수
  let acc = 0.1; //가속도
  ...
  const sceneInfo = [
    {
      //0
      type: "sticky",
      heightNum: 5, //브라우저 5배로 scrollHeight 세팅
      scrollHeight: 0,
      objs: {
    ...
    },
    {
      ...
      values: {
        rect1X: [0, 0, { start: 0, end: 0 }], //디바이스 크기 미정이라 default값 0으로 지정해놓음.
  ...
  function setLayout() {
    ...
    for (let i = 0; i < sceneInfo.length; i++) {
      if (sceneInfo[i].type === "sticky") {
        ...
        sceneInfo[i].scrollHeight = sceneInfo[i].objs.container.offsetHeight; //noraml type는 해당 콘텐츠 height값으로 가져오기
      }
      sceneInfo[
        i
  ...
  function calcValues(values, currentYOffset) {
    let rv;
    // currentScene 에서 스크롤된 범위를 비율로 구하기위한 변수
  ...
  function playAnimation() {
    ...
    const currentYOffset = yOffset - prevScrollHeight; // currentScene 의 스크롤 높이
    const scrollHeight = sceneInfo[currentScene].scrollHeight;
    ...
    switch (currentScene) {
      case 0:
        ...
        // console.log(clacValues(values.messageA_opacity, currentYOffset));
        break;
      case 1:
        //normal type 인터렉션없음
        break;
      case 2:
        ...
        if (scrollRatio > 0.9) {
          ...
          const whiteRectWidth = recalculatedInnerWidth * 0.15; // 좌우 박스 width 15%로 지정
          //좌우 하얀색 박스 위치이동 계산
          ...
          //values.rect1X[0],0,parseInt(whiteRectWidth),objs.canvas.height
          // => ${해당 선택자}, x값,y값, width값, height값
          //가로 영역 그리기
          //fillRect 는 canvas의 영역채워주는 메소드
      ...
      case 3:
        ...
        if (!values.rectStartY) {
          //offset값은 scale이 적용되지 않으므로 계산식을 작성해서 새로 값을 구함.
          values.rectStartY =
  ...
  function loop() {
    ...
    if (!enterNewScene) {
      if (currentScene === 0 || currentScene === 2) {
        const currentYOffset = delayedYOffset - prevScrollHeight; // currentScene 의 스크롤 높이
        const objs = sceneInfo[currentScene].objs;
        const values = sceneInfo[currentScene].values;
        let sequence = Math.round(
          calcValues(values.imageSequence, currentYOffset)
  ...
  //window.addEventListener("DOMContentLoaded", setLayout); == dom요소만 로드되면 실행 실행속도가 load보다 빠름
  window.addEventListener("load", () => {
    ...
    window.addEventListener("resize", () => {
      ...
      sceneInfo[3].values.rectStartY = 0; //일분이 사진 resize시 초기화
    });
